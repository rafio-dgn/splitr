...
| D.4 | KV: ~~hot per-group balance snapshot~~ **each group's recent descriptions, for autofill** ([ADR-0019](../decisions/0019-kv-holds-recent-descriptions-not-balances.md)), rebuildable from D1 | `REQ-D.2` | ✅ 2026-09-24. Miss, hit, loss and invalidation shown; prod KV written via `waitUntil`; [evidence](../evidence/REQ-D.2-kv-recent-descriptions.md) |
| D.5 | R2 **presigned URL** flow: Worker issues the URL, client uploads the photo direct, only the key is stored | `REQ-D.3` | ✅ 2026-09-24. `aws4fetch` with `allHeaders` (the Content-Type pin failed without it), verify-on-attach, [evidence](../evidence/REQ-D.3-presigned-receipt-uploads.md) |
| D.6 | Vision model itemises the receipt into a **draft the user confirms**, never straight into the ledger. **Spike every available vision model on the same ~10 real English receipts**, then write the ADR for the choice ([ADR-0016](../decisions/0016-ai-integration-strategy.md) §2, §10). Manual entry stays the fallback | `REQ-D.3`, ADR-0004 | ✅ 2026-09-24. **Llama 4 Scout** (Raffaele's choice, [ADR-0021](../decisions/0021-receipt-reading-approach.md)), JSON mode, the total confirmed and **enforced on the server**; [evidence](../evidence/D.6-vision-spike.md). Raffaele's own receipts are still owed as confirmation |
| D.7 | Vectorize: **one vector per line item** with `@cf/baai/bge-base-en-v1.5`, `groupId` in the metadata, upsert. Plus the **seed corpus** of about 50 labelled example items for the RAG cold start ([ADR-0016](../decisions/0016-ai-integration-strategy.md) §7, §8) | `REQ-D.4` | ✅ 2026-09-24. Item vectors, plus itemless-expense vectors; item + merchant text; indexed after the save ([ADR-0022](../decisions/0022-semantic-search-design.md)). D1's transaction limit didn't bite: embeddings don't write D1 |
| D.8 | `/search` route: semantic search over past expenses, ~~scoped to the viewer's group~~ **across all the viewer's current groups** (Raffaele, ADR-0022), with item hits grouped back to their expense. **Show it beats keyword** with about 10 labelled queries, semantic vs keyword ([ADR-0016](../decisions/0016-ai-integration-strategy.md) §9) | `REQ-D.4` | ✅ 2026-09-24. **11/11 by meaning against 3/11 by keyword**; across all the viewer's groups; [evidence](../evidence/REQ-D.4-semantic-search.md) |
| D.9 | Let a schema change arise naturally; migrate; update call sites | `REQ-D.5` | ✅ 2026-09-24. `line_item.raw_text`, forced by the C.4 shorthand finding; `0001` applied, `0000` untouched; [evidence](../evidence/REQ-D.5-unanticipated-schema-change.md) |
