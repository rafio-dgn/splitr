...
| D.6 | 🟡 **Spike in progress** ([interim](../evidence/D.6-vision-spike.md): Scout 5/6 totals, Llama 3.2 4/6; waiting on Raffaele's receipts). Vision model itemises the receipt into a **draft the user confirms**, never straight into the ledger. **Spike every available vision model on the same ~10 real English receipts**, then write the ADR for the choice ([ADR-0016](../decisions/0016-ai-integration-strategy.md) §2, §10). Manual entry stays the fallback | `REQ-D.3`, ADR-0004 | ⬜ |
